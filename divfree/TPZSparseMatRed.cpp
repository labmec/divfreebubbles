...
TPZSparseMatRed<TVar,  TSideMatrix>::TPZSparseMatRed () : 
...
template<class TVar, class TSideMatrix>
TPZSparseMatRed<TVar, TSideMatrix>::TPZSparseMatRed( int64_t dim, int64_t dim00 ):
TPZRegisterClassId(&TPZSparseMatRed::ClassId),
...
template<class TVar, class TSideMatrix>
int TPZSparseMatRed<TVar, TSideMatrix>::IsSymmetric() const {
	if(fK00) return this->fK00->IsSymmetric();
	return 0;
}

template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::SimetrizeMatRed() {
...
template<class TVar, class TSideMatrix>
int
TPZSparseMatRed<TVar, TSideMatrix>::PutVal(const int64_t r,const int64_t c,const TVar& value ){
	int64_t row(r),col(c);
...
template<class TVar, class TSideMatrix>
const TVar
TPZSparseMatRed<TVar,TSideMatrix>::GetVal(const int64_t r,const int64_t c ) const {
	int64_t row(r),col(c);
...
template<class TVar, class TSideMatrix>
TVar& TPZSparseMatRed<TVar,TSideMatrix>::s(const int64_t r,const int64_t c ) {
	int64_t row(r),col(c);
...
template<class TVar, class TSideMatrix>
int64_t TPZSparseMatRed<TVar,TSideMatrix>::Size() const
{
...
template<class TVar, class TSideMatrix>
TVar *&TPZSparseMatRed<TVar,TSideMatrix>::Elem()
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZSparseMatRed.\nAborting...\n";
  DebugStop();
...
template<class TVar, class TSideMatrix>
const TVar *TPZSparseMatRed<TVar,TSideMatrix>::Elem() const
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZSparseMatRed.\nAborting...\n";
  DebugStop();
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar,TSideMatrix>::SetSolver(TPZAutoPointer<TPZMatrixSolver<TVar> > solver)
{
	fK00=solver->Matrix();
	fSolver = solver;
}
...
template<class TVar,class TSideMatrix>
void
TPZSparseMatRed<TVar, TSideMatrix>::SetK00(TPZAutoPointer<TPZMatrix<TVar> > K00)
{
	fK00=K00;
}

template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar,TSideMatrix>::SetF(const TPZFMatrix<TVar> & F)
{
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::F1Red(TPZFMatrix<TVar> &F1Red)
{
	if (!fDim0)
    {
        F1Red = fF1;
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar,TSideMatrix>::K11Reduced(TPZFMatrix<TVar> &K11, TPZFMatrix<TVar> &F1)
{
...
template<>
void TPZSparseMatRed<double, TPZVerySparseMatrix<double> >::K11Reduced(TPZFMatrix<double> &K11, TPZFMatrix<double> &F1)
{
...
template<>
void TPZSparseMatRed<float, TPZVerySparseMatrix<float> >::K11Reduced(TPZFMatrix<float> &K11, TPZFMatrix<float> &F1)
{
	std::cout << __PRETTY_FUNCTION__ << " should never be called\n";	
...
template<>
void TPZSparseMatRed<long double, TPZVerySparseMatrix<long double> >::K11Reduced(TPZFMatrix<long double> &K11, TPZFMatrix<long double> &F1)
{
	std::cout << __PRETTY_FUNCTION__ << " should never be called\n";	
...
template<>
void TPZSparseMatRed<std::complex<double>, TPZVerySparseMatrix<std::complex<double> > >::K11Reduced(TPZFMatrix<std::complex<double> > &K11, TPZFMatrix<std::complex<double> > &F1)
{
	std::cout << __PRETTY_FUNCTION__ << " should never be called\n";	
...
template<class TVar ,class TSideMatrix>
void TPZSparseMatRed<TVar,TSideMatrix>::U1(TPZFMatrix<TVar> & F)
{
...
template<>
void TPZSparseMatRed<REAL, TPZVerySparseMatrix<REAL> >::UGlobal(const TPZFMatrix<REAL> & U1, TPZFMatrix<REAL> & result)
{
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix >::UGlobal(const TPZFMatrix<TVar> & U1, TPZFMatrix<TVar> & result)
{
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::UGlobal2(TPZFMatrix<TVar> & U1, TPZFMatrix<TVar> & result)
{
	TPZFMatrix<TVar> u0( fF0.Rows() , fF0.Cols() );
...
template<class TVar,class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::Print(const char *name , std::ostream &out ,const MatrixOutputFormat form ) const
{	
	if(form != EInputFormat) {
		out << "Writing matrix 'TPZSparseMatRed<TSideMatrix>::" << name;
...
template<class TVar, class TSideMatrix>
int TPZSparseMatRed<TVar,TSideMatrix>::Redim(int64_t dim, int64_t dim00){
	if(dim<dim00) TPZMatrix<TVar>::Error(__PRETTY_FUNCTION__,"dim k00> dim");
...
template<class TVar, class TSideMatrix>
int TPZSparseMatRed<TVar, TSideMatrix>::Zero(){
	if(fK00) fK00->Zero();
    fIsReduced = false;
    fK01IsComputed = false;
    fF0IsComputed = false;
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::MultAdd(const TPZFMatrix<TVar> &x,
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::DecomposeK00()
{
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::Write(TPZStream &buf, int withclassid) const {
...
template<class TVar, class TSideMatrix>
void TPZSparseMatRed<TVar, TSideMatrix>::Read(TPZStream &buf, void *context) {
...
template class TPZSparseMatRed<double, TPZVerySparseMatrix<double> >;
template class TPZSparseMatRed<double, TPZFMatrix<double> >;
