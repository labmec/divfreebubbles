...
public:
	...
	TPZSparseMatRed<TVar ,TSideMatrix>(const TPZSparseMatRed<TVar, TSideCopy> &cp): TPZMatrix<TVar>(cp), fK11(cp.fK11), fK01(cp.fK01), fK10(cp.fK10), fF0(cp.fF0), fF1(cp.fF1),fMaxRigidBodyModes(cp.fMaxRigidBodyModes),fNumberRigidBodyModes(cp.fNumberRigidBodyModes), fF0IsComputed(cp.fF0IsComputed)
	{
		fDim0=cp.fDim0;
		fDim1=cp.fDim1;
  ...
  {                                                           
    auto *from = dynamic_cast<const TPZSparseMatRed<TVar,TSideMatrix> *>(mat);                
    if (from) {                                               
      *this = *from;                                          
    }                                                         
    else                                                      
	...
	virtual void K11Reduced(TPZFMatrix<TVar> &K11, TPZFMatrix<TVar> &F1);
...
template<class TVar, class TSideMatrix>
inline void TPZSparseMatRed<TVar, TSideMatrix>::Swap(int64_t *a, int64_t *b)
{
	int64_t aux = *a;
	*a = *b;
	*b = aux;
