/**
 * @file
 * @brief Contains the implementation of the TPZDoubleMatRed methods.
 */
...
template<class TVar>
TPZDoubleMatRed<TVar>::TPZDoubleMatRed () : 
...
template<class TVar>
TPZDoubleMatRed<TVar>::TPZDoubleMatRed( int64_t dim, int64_t dim00 ):
TPZRegisterClassId(&TPZDoubleMatRed::ClassId),
...
template<class TVar>
int64_t TPZDoubleMatRed<TVar>::Size() const
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZDoubleMatRed.\nAborting...\n";
  DebugStop();
  return -1;
}
template<class TVar>
TVar *&TPZDoubleMatRed<TVar>::Elem()
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZDoubleMatRed.\nAborting...\n";
  DebugStop();
...
template<class TVar>
const TVar *TPZDoubleMatRed<TVar>::Elem() const
{
  PZError<<__PRETTY_FUNCTION__;
  PZError<<"\nERROR: should not be called for TPZDoubleMatRed.\nAborting...\n";
  DebugStop();
...
template<class TVar>
void TPZDoubleMatRed<TVar>::SetSolver(TPZAutoPointer<TPZMatrixSolver<TVar> > solver)
{
...
template<class TVar>
void TPZDoubleMatRed<TVar>::SetF(const TPZFMatrix<TVar> & F)
{
...
template<class TVar>
void TPZDoubleMatRed<TVar>::F1Red(TPZFMatrix<TVar> &F1Red)
{
	if (!fDim0)
    {
        F1Red = fF1;
...
template<class TVar>
void TPZDoubleMatRed<TVar>::K11Reduced(TPZFMatrix<TVar> &K11, TPZFMatrix<TVar> &F1)
{
  	TPZTimer clock;
...
template<class TVar>
void TPZDoubleMatRed<TVar>::U1(TPZFMatrix<TVar> & F)
{
...
template<class TVar>
void TPZDoubleMatRed<TVar>::UGlobal(const TPZFMatrix<TVar> & U1, TPZFMatrix<TVar> & result)
{
...
template<class TVar>
void TPZDoubleMatRed<TVar>::UGlobal2(TPZFMatrix<TVar> & U1, TPZFMatrix<TVar> & result)
{
	TPZFMatrix<TVar> u0( fF0.Rows() , fF0.Cols() );
...
template<class TVar>
void TPZDoubleMatRed<TVar>::Print(const char *name , std::ostream &out ,const MatrixOutputFormat form ) const
{	
	if(form != EInputFormat) {
		out << "Writing matrix 'TPZDoubleMatRed::" << name;
...
template<class TVar>
int TPZDoubleMatRed<TVar>::Redim(int64_t dim, int64_t dim00){
	if(dim<dim00) TPZMatrix<TVar>::Error(__PRETTY_FUNCTION__,"dim k00> dim");
...
template<class TVar>
void TPZDoubleMatRed<TVar>::MultAdd(const TPZFMatrix<TVar> &x,
									...
									const int opt) const
{
	...
	if(!fIsReduced)
	{
		LOGPZ_ERROR(logger,"TPZDoubleMatRed not reduced, expect trouble")
...
template<class TVar>
void TPZDoubleMatRed<TVar>::DecomposeK00()
{
    if(fK00->IsDecomposed())
    {
        return;
...
template<class TVar>
void TPZDoubleMatRed<TVar>::Write(TPZStream &buf, int withclassid) const {
...
template<class TVar>
void TPZDoubleMatRed<TVar>::Read(TPZStream &buf, void *context) {
...
template<class TVar>
void TPZDoubleMatRed<TVar>::ReorderEquations(TPZCompMesh *cmesh, std::set<int> &LagLevels, int64_t &dim, int64_t &dim00) {
...
template<class TVar>
void TPZDoubleMatRed<TVar>::AllocateSubMatrices(TPZCompMesh *cmesh) {
    ...
    // K10 is skiped because the transposition is performed inside TPZDoubleMatRed, so here we insert a zero vector.
...
template class TPZDoubleMatRed<double>;
template class TPZDoubleMatRed<float>;
