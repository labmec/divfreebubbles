/**
 * @file
 * @brief Contains TPZDoubleMatRed class which implements a simple substructuring of a linear system of equations, composed of 4 submatrices.
 */

#ifndef _TDOUBLEMATREDHH_
#define _TDOUBLEMATREDHH_
...
template<class TVar >
class TPZDoubleMatRed: public TPZMatrix<TVar>
{
public:
	
	// friend class TPZDoubleMatRed<TVar, TPZFMatrix<TVar> >;
	// friend class TPZDoubleMatRed<TVar ,TPZVerySparseMatrix<TVar> >;
	// friend class TPZDoubleMatRed<TVar ,TPZSYsmpMatrix<TVar> >;
	// friend class TPZDoubleMatRed<TVar ,TPZFYsmpMatrix<TVar> >;
	 ...
	 */
	TPZDoubleMatRed(const int64_t dim, const int64_t dim00);

	TPZDoubleMatRed(TPZCompMesh *cmesh, std::set<int> &LagLevels);

	template<class TSideCopy>
	TPZDoubleMatRed<TVar>(const TPZDoubleMatRed<TVar> &cp): TPZMatrix<TVar>(cp), fK11(cp.fK11), fK01(cp.fK01), fK10(cp.fK10), fF0(cp.fF0), fF1(cp.fF1),fMaxRigidBodyModes(cp.fMaxRigidBodyModes),fNumberRigidBodyModes(cp.fNumberRigidBodyModes), fF0IsComputed(cp.fF0IsComputed)
	{
		fDim0=cp.fDim0;
		fDim1=cp.fDim1;
  ...
  inline TPZDoubleMatRed<TVar>*NewMatrix() const override {return new TPZDoubleMatRed<TVar>{};}
  CLONEDEF(TPZDoubleMatRed)
  
  /** @brief Creates a copy from another TPZDoubleMatRed*/
  void CopyFrom(const TPZMatrix<TVar> *  mat) override
  {
    auto *from = dynamic_cast<const TPZDoubleMatRed<TVar> *>(mat);
    if (from) {
      *this = *from;
    }
    else
...
template<class TVar>
int TPZDoubleMatRed<TVar>::ClassId() const{
    return Hash("TPZDoubleMatRed") ^ TPZMatrix<TVar>::ClassId()  << 2;
}
...
template<class TVar>
inline void TPZDoubleMatRed<TVar>::Swap(int64_t *a, int64_t *b)
{
	int64_t aux = *a;
	*a = *b;
	*b = aux;
