...
std::ofstream rprint("Condition_number.txt",std::ofstream::out);
...
// inline std::string ApproxSpaceKernelHDiv_Name(TPZHDivApproxCreator<STATE>::MSpaceType approxSpace)
// {
// 	switch (approxSpace)
// 	{
// 		case TPZHDivApproxCreator<STATE>::ENone:
// 		{
// 			return "ENone";
// 		}
// 		case TPZHDivApproxCreator<STATE>::EFullHybrid:
// 		{
// 			return "EFullHybrid";
// 		}
// 		case TPZHDivApproxCreator<STATE>::ESemiHybrid:
// 		{
// 			return "ESemiHybrid";
// 		}
// 		case TPZHDivApproxCreator<STATE>::EDuplicatedConnects:
// 		{
// 			return "EDuplicatedConnects";
// 		}
// 		default:
//         {
//             return "TPZHDivApproxCreator not found!";
//         }
//     }
//     DebugStop();
// 	return "";
...
int main( int argc, char* argv[] )
{
    ...
    // TPZHDivApproxCreator<STATE>::MSpaceType approxSpace = GENERATE(TPZHDivApproxCreator<STATE>::EFullHybrid);
...
void TestHybridization(const int &xdiv, const int &pOrder, HDivFamily &hdivfamily)
{
    ...
    TPZHDivApproxCreator createSpace(gmesh);
    createSpace.HdivFamily() = hdivfamily;
    ...
    rprint << "Polynomial order = " << pOrder << "\n " ;
    rprint << "Number of equations = " << nEquationsFull << "\n " ;

    TPZTimer clock,clock2;
    clock.start();

    // Group and condense the elements
    if (approxSpace == TPZHDivApproxCreator<STATE>::EDuplicatedConnects){
    ...
    an.SetComputeEigenvectors(false);

    an.Assemble();
    an.Solve();

    auto eigenval = an.GetEigenvalues();
    ...
    {
        if (fabs(eigenval[i].real()) > maxval) maxval = eigenval[i].real();
        if (fabs(eigenval[i].real()) < minval) minval = eigenval[i].real();
        rprint << eigenval[i] << std::endl;
    }
    rprint << "Condition number = " << maxval/minval << std::endl;
    rprint << std::endl;

    // std::cout << "Sol = " << eigenval << std::endl;
    // //Solve problem
    // if (approxSpace == TPZHDivApproxCreator<STATE>::EDuplicatedConnects){
